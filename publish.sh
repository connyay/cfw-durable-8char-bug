...
upload_script() {
  curl_api https://api.cloudflare.com/client/v4/accounts/$ACCOUNT_ID/workers/scripts/$SCRIPT_NAME \
      ...
      -F "script=@ocho.mjs;type=application/javascript+module" > /dev/null
}
...
upload_bootstrap_script() {
  echo '{"main_module": "ocho.mjs"}' > bootstrap-metadata.json
  curl_api https://api.cloudflare.com/client/v4/accounts/$ACCOUNT_ID/workers/scripts/$SCRIPT_NAME \
      -X PUT \
...
if [ ! -e metadata.json ]; then
  ...
  upload_bootstrap_script
  OCHO_ID=$(upsert_namespace Ocho)

  cat > metadata.json << __EOF__
{
  "main_module": "ocho.mjs",
  "bindings": [
    {
      "type": "durable_object_namespace",
      "name": "ocho",
      "namespace_id": "$OCHO_ID"
    }
  ]
}
__EOF__
